...
#define VP8_MAX_QUANTIZER 63
#define MMSET(x) memset(&(x),0,sizeof(x))

vp8_encoder vp8Settings = VP8_DEFAULT_CONF;

/**
 *  \fn vp8Encoder
 */
vp8Encoder::vp8Encoder(ADM_coreVideoFilter *src, bool globalHeader) : ADM_coreVideoEncoder(src)
{
    ADM_info("Creating VP8 encoder\n");
    MMSET(context);
    MMSET(param);
    iface=NULL;
    pic=NULL;
...
bool vp8Encoder::setup(void)
{
    ...
    if(!iface)
    {
        ADM_error("No VP8 interface available.\n");
        return false;
    }
    ...
    if(ret!=VPX_CODEC_OK)
    {
        ADM_error("[vp8Encoder] Cannot set default configuration, error %d: %s.\n",(int)ret,vpx_codec_err_to_string(ret));
        return false;
    }
    ...
    param.g_w=getWidth();
    param.g_h=getHeight();
    uint32_t threads=vp8Settings.autoThreads? ADM_cpu_num_processors() : vp8Settings.nbThreads;
    if(threads>VP8_ENC_MAX_THREADS) threads=VP8_ENC_MAX_THREADS;
    param.g_threads=vp8Settings.nbThreads=threads;
...
#define MAX_CLOCK 180000
    ...
    int speed=(vp8Settings.speed > 18)? 9 : vp8Settings.speed-9;

    param.rc_min_quantizer=vp8Settings.ratectl.qz;
    param.rc_max_quantizer=VP8_MAX_QUANTIZER;
    switch(vp8Settings.ratectl.mode)
    {
        case COMPRESS_CQ:
            param.rc_max_quantizer=vp8Settings.ratectl.qz;
            param.rc_end_usage=VPX_CQ;
            break;
        case COMPRESS_CBR:
            param.rc_target_bitrate=vp8Settings.ratectl.bitrate;
        ...
        case COMPRESS_2PASS_BITRATE:
            ...
            ADM_info("[vp8Encoder] Starting pass %d\n",passNumber);
            if(passNumber==1)
            {
            ...
            {
                uint32_t bitrate=0;
                if(vp8Settings.ratectl.mode==COMPRESS_2PASS)
                {
                    uint64_t duration=source->getInfo()->totalDuration;
                    if(vp8Settings.ratectl.finalsize)
                    {
                        if(false==ADM_computeAverageBitrateFromDuration(duration, vp8Settings.ratectl.finalsize, &bitrate))
                            return false;
                    }
                }else
                {
                    bitrate=vp8Settings.ratectl.avg_bitrate;
                }
                if(bitrate)
                {
    ...
    param.kf_max_dist=vp8Settings.keyint;
    ...
    if(ret!=VPX_CODEC_OK)
    {
        ADM_error("[vp8Encoder] Init failed with error %d: %s\n",(int)ret,vpx_codec_err_to_string(ret));
        return false;
    }
    ...
    if(!pic)
    {
        ADM_error("[vp8Encoder] Cannot allocate VPX image.\n");
        return false;
    }

    dline=VPX_DL_GOOD_QUALITY;
    switch(vp8Settings.deadline)
    {
        case 0:
            if(passNumber==1)
                break;
    ...
    {
        ADM_warning("[vp8Encoder] Cannot set VP8E_SET_CPUUSED codec control to %d\n",speed);
    }
    if(param.rc_end_usage==VPX_CQ && VPX_CODEC_OK!=vpx_codec_control(&context,VP8E_SET_CQ_LEVEL,vp8Settings.ratectl.qz))
    {
        ADM_warning("[vp8Encoder] Cannot set VP8E_SET_CQ_LEVEL codec control to %u\n",vp8Settings.ratectl.qz);
    }
    return true;
}
 ...
 */
bool vp8Encoder::setPassAndLogFile(int pass, const char *name)
{
...
/**
    \fn encode
*/
bool vp8Encoder::encode(ADMBitstream *out)
{
    // 1 fetch a frame...
    uint32_t nb;
    uint64_t pts;
...
again:
    ...
    {
        ADM_warning("[vp8] Cannot get next image\n");
        flush=true;
    }
    if(flush)
    {
...
*/
bool vp8Encoder::isDualPass(void)
{
    uint32_t encmode=vp8Settings.ratectl.mode;
...
*/
bool vp8Encoder::postAmble(ADMBitstream *out)
{
    ...
    if(packetQueue.size())
    {
        ...
        if(passNumber!=1)
        {
            ...
            { // The value filled by codec control does not apply to this particular frame, but oh well...
                if(quantizer<=0) quantizer=vp8Settings.ratectl.qz;
