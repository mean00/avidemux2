...
uint8_t  asfHeader::getFrame(uint32_t framenum,ADMCompressedImage *img)
{
    ...
    if (framenum >= nbImage)
    {
        ADM_warning("Requested frame %u out of bounds, must be < %u\n", framenum, nbImage);
        return 0;
    }
    asfIndex *dx = &(_index[framenum]);
    if (dx->frameLen > ADM_COMPRESSED_MAX_DATA_LENGTH)
    {
        ADM_error("Frame %u size %u exceeds max. supported.\n", framenum, dx->frameLen);
        return 0;
    }
    if (!dx->frameLen)
    {
    ...
    curSeq&=0xff;
    aprintf("Framenum %u len: %u curSeq: %u frameSeq: %u packetnb: %llu\n",
            framenum, dx->frameLen, curSeq, dx->segNb, dx->packetNb);
    // Seeking ?
    if (dx->segNb != curSeq || dx->segNb == 1)
    {
        aprintf("Seeking... curSeq: %u wanted seq: %u packet: %" PRIu64"\n", curSeq, dx->segNb, dx->packetNb);
        if(!_packet->goToPacket(dx->packetNb))
        {
            ADM_error("Cannot seek to frame %u\n", framenum);
            return 0;
        }
        _packet->purge();
        curSeq = dx->segNb;
        ADM_info("Seeking done, starting at seq %u \n", curSeq);
    }
    ...
    while(1)
    {
        while(readQueue.size())
        {
            ...
            bit = readQueue.front();
            readQueue.pop_front();
            aprintf("-> found packet of size %d seq %d while curseq = %d wanted seg = %u, acc. data len: %u, packet: %u\n",
                    bit->len, bit->sequence, curSeq, dx->segNb, len, bit->packet);
            // Here it is tricky
            ...
            delta = 256 + bit->sequence - dx->segNb;
            delta &= 0xff;
            if (!len) // Starting a new frame
            {
                if (dx->segNb != bit->sequence)
                {
                    aprintf("Dropping seq %u, too old for seg %u, delta: %d\n", bit->sequence, dx->segNb, delta);
                    storageQueue.push_back(bit);
                    bit = NULL;
                    if (delta < 230)
                    {
                        aprintf("[ASF] Very suspicious delta : %" PRIu32"\n", delta);
                    }
                    continue;
                }
                // We have found our first chunk
                curSeq = bit->sequence;
                ADM_assert(bit->len <= dx->frameLen);
            ...
            {
                aprintf("New sequence %u -> %u while loading frame %u\n", curSeq, bit->sequence, framenum);
                readQueue.push_front(bit);
                curSeq = bit->sequence;
                goto gotcha;
            }
            // still same sequence ...add
            ADM_assert((bit->len <= dx->frameLen) && (len + bit->len <= dx->frameLen));
        ...
        if(_packet->finished())
        {
            ADM_warning("Too close to EOF.\n");
            return 0;
        }
        if(!_packet->nextPacket(_videoStreamId))
        {
            ADM_error("Packet error.\n");
            return 0;
        }
        _packet->skipPacket();
    }
gotcha:
    ...
    if (len != dx->frameLen)
    {
        ADM_error("Frame %u size mismatch: got %u, expected %u\n", framenum, len, dx->frameLen);
        return 0;
    }
    img->dataLength = len;
