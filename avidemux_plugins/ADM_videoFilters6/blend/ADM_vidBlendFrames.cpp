...
DECLARE_VIDEO_FILTER(AVDM_BlendFrames, 1, 0, 0,                  // Version
                     ADM_UI_ALL,                                 // UI
                     VF_TRANSFORM,                               // Category
                     "blend",                                    // internal name (must be uniq!)
                     QT_TRANSLATE_NOOP("blend", "Blend Frames"), // Display name
                     QT_TRANSLATE_NOOP("blend", "Blend groups of N frames into a single frame.  Useful for speeding up "
                                                "slow motion footage or creating timelapses.") // Description
);
/**
 * \fn configure
...
{
#define MAX_BLEND_FRAMES                                                                                               \
    16777216 // 2^32/2^8 This is the in-all-cases limit, but on average it should be able to support probably 25% more.
             // However, no frames will be exported unless the number of frames in the video is equal or greater.
    ...
    {
        info.totalDuration = previousFilter->getInfo()->totalDuration /
                             ((uint64_t)param.N); // This bad boy reports the proper duration to the loading bar
    ...
    while (true)
    {
        ...
        if (buffer == NULL)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                // I know that there is some way to initialize this with zeroes more efficiently, but I don't know how
                // to do it.
                for (int y = 0; y < h; y++)
                {
                    for (int x = 0; x < w; x++)
        ...
        if (accumulated == param.N)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                for (int y = 0; y < h; y++)
                {
                    for (int x = 0; x < w; x++)
                    {
                        ip[x] = (uint8_t)(buffer[i][y * w + x] /
                                          (uint32_t)param.N); // Not sure if this will cast weirdly//It casted weirdly
                                                              // and made everything green, fixed now
