...
void adm_yadif_filter_line_sse2(uint8_t *dst, const uint8_t *prev, const uint8_t *cur, const uint8_t *next, int w, int refs, int parity,int mode );
...
/**
    \class yadifFilter
*/
class ffyadifFilter : public  ADM_coreVideoFilterCached
{
protected:
                    ADMImage    *original;
                    ffyadif     configuration;
                    void        updateInfo(void);
public:
                    ffyadifFilter(ADM_coreVideoFilter *previous,CONFcouple *conf);
                    ~ffyadifFilter();
...
static void filter_line_c( uint8_t *dst, const uint8_t *prev, const uint8_t *cur, const uint8_t *next, int w, int refs, int parity,int mode);
// Add the hook to make it valid plugin
DECLARE_VIDEO_FILTER(   ffyadifFilter,   // Class
                        1,0,0,              // Version
                        ADM_UI_ALL,         // UI
                        VF_INTERLACING,            // Category
                        "ffyadif",            // internal name (must be uniq!)
                        QT_TRANSLATE_NOOP("ffyadif","FFYadif"),            // Display name
                        QT_TRANSLATE_NOOP("ffyadif","Yadif, port of ffmpeg version.") // Description
                    );
...
/**
    \fn constructor
*/
ffyadifFilter::ffyadifFilter(ADM_coreVideoFilter *in, CONFcouple *setup): 
            ADM_coreVideoFilterCached(10,in,setup)
{
    original=new ADMImageDefault(in->getInfo()->width,in->getInfo()->height);
    if(!setup || !ADM_paramLoad(setup,ffyadif_param,&configuration))
    {
        // Default value
        configuration.mode=0;
        configuration.order=1;
        configuration.deint=1;
    }
...
void ffyadifFilter::updateInfo(void)
{
...
/**
    \fn updateInfo
*/
bool ffyadifFilter::configure( void) 
{
...
*/
bool         ffyadifFilter::getCoupledConf(CONFcouple **couples)
{
    return ADM_paramSave(couples, ffyadif_param,&configuration);
}

void ffyadifFilter::setCoupledConf(CONFcouple *couples)
{
    ADM_paramLoad(couples, ffyadif_param, &configuration);
}
...
*/
const char *ffyadifFilter::getConfiguration(void)
{
    static char conf[80];
    conf[0]=0;
...
/**
    ...
    \brief Return current setting as a string
*/
bool ffyadifFilter::getNextFrame(uint32_t *fn,ADMImage *image)
{
...
void filter_line_c( uint8_t *dst, const uint8_t *prev, const uint8_t *cur, const uint8_t *next, int w, int refs, int parity,int mode)
{
    int x;
...
{
        void (*filter_line)( uint8_t *dst, const uint8_t *prev, const uint8_t *cur, const uint8_t *next, int w, int refs, int parity,int mode);
	int y;
	...
	if (CpuCaps::hasSSE2()) 
        ...
        for(y=2; y<h-1; y++){
            if(((y ^ parity) & 1)){
                ...
                filter_line( dst2, prev, cur, next, w, refs, (parity ^ tff),mode);
            }else{
