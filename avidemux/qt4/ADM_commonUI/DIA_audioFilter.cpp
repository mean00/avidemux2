...
int DIA_getAudioFilter(ADM_AUDIOFILTER_CONFIG *config, double tempoHint)
{
    ...
    ELEM_TYPE_FLOAT drcFloorDB =
        10.0 * std::log10(config->drcConf.mFloor); // convert to DB; factor of 10 because it's power
...
#define PX(x) (&(config->x))
    diaElemToggleUint eResample(PX(resamplerEnabled), QT_TRANSLATE_NOOP("adm", "R_esampling (Hz):"),
                                PX(resamplerFrequency), QT_TRANSLATE_NOOP("adm", "Resampling frequency (Hz)"), 6000,
                                64000);

    //**********************************
    diaMenuEntry menuFPS[] = {{FILMCONV_NONE, QT_TRANSLATE_NOOP("adm", "None"), NULL},
...
#define NB_ITEMS(x) sizeof(x) / sizeof(diaMenuEntry)
    ...
    //**********************************
    diaMenuEntry menuMixer[] = {{CHANNEL_INVALID, QT_TRANSLATE_NOOP("adm", "No change"), NULL},
    ...
    //*************************
    diaMenuEntry menuGain[] = {{ADM_NO_GAIN, QT_TRANSLATE_NOOP("adm", "None"), NULL},
    ...
    //****************************
    diaElemToggleInt eShift(&bShiftEnabled, QT_TRANSLATE_NOOP("adm", "Shift audio (ms):"), &vShift,
                            QT_TRANSLATE_NOOP("adm", "Shift Value (ms):"), -30000, 30000);
/************************************/
#define NB_ELEM(x) sizeof(x) / sizeof(diaElem *)
    ...
    char *gainTxt = ADM_strdup(QT_TRANSLATE_NOOP(
        "adm", "<hr><p>It is highly recommended to enable normalization when using positive gain values</p>"));
    ...
    diaElemFloat eChGainFCValue(chGainDB + ADM_CH_FRONT_CENTER, QT_TRANSLATE_NOOP("adm", "Front center (dB):"), -30,
                                +30);
    ...
    diaElemFloat eChGainLFEValue(chGainDB + ADM_CH_LFE, QT_TRANSLATE_NOOP("adm", "Low-frequency effects (LFE) (dB):"),
                                 -30, +30);
    ...
    diaElem *chanGainsElems[] = {&eChGainFLValue, &eChGainFRValue, &eChGainFCValue, &eChGainSLValue,  &eChGainSRValue,
                                 &eChGainRLValue, &eChGainRRValue, &eChGainRCValue, &eChGainLFEValue, &noteChGain};
    diaElemTabs tabChanGains(QT_TRANSLATE_NOOP("adm", "Channel gains"), NB_ELEM(chanGainsElems), chanGainsElems);

    //*** Channel delays tab ******
    diaElemInteger eChDelayFLValue(chDelayMS + ADM_CH_FRONT_LEFT, QT_TRANSLATE_NOOP("adm", "Front left (ms):"), 0,
                                   10000);
    diaElemInteger eChDelayFRValue(chDelayMS + ADM_CH_FRONT_RIGHT, QT_TRANSLATE_NOOP("adm", "Front right (ms):"), 0,
                                   10000);
    diaElemInteger eChDelayFCValue(chDelayMS + ADM_CH_FRONT_CENTER, QT_TRANSLATE_NOOP("adm", "Front center (ms):"), 0,
                                   10000);
    diaElemInteger eChDelayRLValue(chDelayMS + ADM_CH_REAR_LEFT, QT_TRANSLATE_NOOP("adm", "Rear left (ms):"), 0, 10000);
    diaElemInteger eChDelayRRValue(chDelayMS + ADM_CH_REAR_RIGHT, QT_TRANSLATE_NOOP("adm", "Rear right (ms):"), 0,
                                   10000);
    diaElemInteger eChDelayRCValue(chDelayMS + ADM_CH_REAR_CENTER, QT_TRANSLATE_NOOP("adm", "Rear center (ms):"), 0,
                                   10000);
    diaElemInteger eChDelaySLValue(chDelayMS + ADM_CH_SIDE_LEFT, QT_TRANSLATE_NOOP("adm", "Side left (ms):"), 0, 10000);
    diaElemInteger eChDelaySRValue(chDelayMS + ADM_CH_SIDE_RIGHT, QT_TRANSLATE_NOOP("adm", "Side right (ms):"), 0,
                                   10000);
    diaElemInteger eChDelayLFEValue(chDelayMS + ADM_CH_LFE,
                                    QT_TRANSLATE_NOOP("adm", "Low-frequency effects (LFE) (ms):"), 0, 10000);

    diaElemReadOnlyText noteChDelay(
        NULL,
        QT_TRANSLATE_NOOP("adm", "<hr><p>The final delay will be the sum of a value above and the \"Shift audio\" "
                                 "value provided on the Main tab</p>"),
        NULL);

    diaElem *chanDelaysElems[] = {&eChDelayFLValue,  &eChDelayFRValue, &eChDelayFCValue, &eChDelaySLValue,
                                  &eChDelaySRValue,  &eChDelayRLValue, &eChDelayRRValue, &eChDelayRCValue,
                                  &eChDelayLFEValue, &noteChDelay};
    ...
    diaMenuEntry menuRemap[] = {{0, QT_TRANSLATE_NOOP("adm", "Front left"), NULL},
    ...
    diaElemMenu eRemapLFE(vChRemap + 8, QT_TRANSLATE_NOOP("adm", "Low-frequency effects (LFE) to:"),
                          NB_ITEMS(menuRemap), menuRemap);
    ...
    diaElemReadOnlyText noteRemap(
        NULL,
        QT_TRANSLATE_NOOP("adm", "<hr><p>Remapping does not change the channel layout, therefore:</p><ul>"
    ...
    diaElem *remapElems[] = {&eChRemap, &eRemapFL, &eRemapFR, &eRemapFC,  &eRemapSL, &eRemapSR,
                             &eRemapRL, &eRemapRR, &eRemapRC, &eRemapLFE, &noteRemap};
    diaElemTabs tabRemap(QT_TRANSLATE_NOOP("adm", "Channel remap"), NB_ELEM(remapElems), remapElems);

    //*** ALL TABS *************************
    diaElemTabs *tabs[] = {&tabMain, &tabDRC, &tabEq, &tabChanGains, &tabChanDelays, &tabRemap};
