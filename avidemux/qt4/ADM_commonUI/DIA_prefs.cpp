...
uint8_t DIA_Preferences(void)
{
...
#endif
    bool hzd, vzd, dring;
    bool capsMMX, capsMMXEXT, caps3DNOW, caps3DNOWEXT, capsSSE, capsSSE2, capsSSE3, capsSSSE3, capsSSE4, capsSSE42,
        capsAVX, capsAVX2, capsFMA3, capsAll;
    ...
    if (!prefs->get(DEFAULT_POSTPROC_TYPE, &pp_type))
        pp_type = 0;
    if (!prefs->get(DEFAULT_POSTPROC_VALUE, &pp_value))
        pp_value = 0;
...
#define CPU_CAPS(x)                                                                                                    \
    ...
    if (!prefs->get(HDR_TONEMAPPING, &toneMappingHDR))
        toneMappingHDR = 1;
    if (!prefs->get(HDR_TARGET_LUMINANCE, &targetLumHDR))
        targetLumHDR = DEFAULT_TARGET_LUMINANCE_HDR;
    if (!prefs->get(HDR_OUT_OF_GAMUT_HANDLING, &outOfGamutHDR))
        outOfGamutHDR = 0;
    ...
    if (!prefs->get(FEATURES_MPEG_NO_LIMIT, &mpeg_no_limit))
        mpeg_no_limit = 0;
    ...
    // Audio device
    /************************ Build diaelems ****************************************/
    diaElemToggle allowAnyMpeg(&mpeg_no_limit,
                               QT_TRANSLATE_NOOP("adm", "_Accept non-standard audio frequency for DVD"));
    diaElemToggle resetEncoder(&loadDefault,
                               QT_TRANSLATE_NOOP("adm", "_Revert to saved default output settings on video load"));
    diaElemToggle enableAltShortcuts(&altKeyboardShortcuts,
                                     QT_TRANSLATE_NOOP("adm", "_Enable alternative keyboard shortcuts"));
    diaElemToggle swapUpDownKeys(&swapUpDown,
                                 QT_TRANSLATE_NOOP("adm", "Re_verse UP and DOWN arrow keys for navigation"));
    diaElemToggle swapMouseWheel(&swapWheel, QT_TRANSLATE_NOOP("adm", "Reverse mouse _wheel for navigation"));
    diaElemToggle swapMarkers(
        &useSwap,
        QT_TRANSLATE_NOOP("adm", "_Swap markers if marker A is set past marker B or marker B before A in video"));
...
#undef CPU_CAP_AVAIL
    ...
    diaElemReadOnlyText lavcMultiThreadHwText(
        NULL,
...
#define NB_ITEMS(x) sizeof(x) / sizeof(diaMenuEntry)
    ...
    diaElemMenu menuEncodePriority(&encodePriority, QT_TRANSLATE_NOOP("adm", "_Encoding priority:"),
                                   NB_ITEMS(priorityEntries), priorityEntries);
    framePriority.swallow(&menuEncodePriority);
    // diaElemMenu menuIndexPriority(&indexPriority, QT_TRANSLATE_NOOP("adm","_Indexing/unpacking priority:"),
    // NB_ITEMS(priorityEntries), priorityEntries); framePriority.swallow(&menuIndexPriority);
#ifdef _WIN32
    diaElemMenu menuPlaybackPriority(&playbackPriority, QT_TRANSLATE_NOOP("adm", "_Playback priority:"),
                                     NB_ITEMS(priorityEntries), priorityEntries);
    framePriority.swallow(&menuPlaybackPriority);
#else
    diaElemReadOnlyText encodePriorityNixText(
        NULL,
        /* advanced users know how to renice, we cannot do this ourselves */
        QT_TRANSLATE_NOOP("adm",
                          "Priority can be only lowered and applies to the entire application for the current session"),
        NULL);
    framePriority.swallow(&encodePriorityNixText);
#endif

    diaElemToggle useLastReadAsTarget(
        &lastReadDirAsTarget, QT_TRANSLATE_NOOP("adm", "_Default to the directory of the last read file for saving"));
    diaElemToggle firstPassLogFilesAutoDelete(&multiPassStatsAutoDelete,
                                              QT_TRANSLATE_NOOP("adm", "De_lete first pass log files by default"));

    // Multiload
    diaElemFrame frameMultiLoad(QT_TRANSLATE_NOOP("adm", "Auto-Append Settings"));
    diaElemToggle multiLoadUseCustomFragmentSize(
        &useCustomFragmentSize, QT_TRANSLATE_NOOP("adm", "_Use custom fragment size for auto-append of MPEG-TS files"));
    diaElemUInteger multiLoadCustomFragmentSize(&customFragmentSize, QT_TRANSLATE_NOOP("adm", "_Fragment size:"), 250,
                                                8196);
    ...
    diaElemToggle toggleReversePicsOrder(
        &loadPicsInReverseOrder, QT_TRANSLATE_NOOP("adm", "_Load sequentially named pictures in reverse order"));
    diaMenuEntry standardFpsEntries[] = {{0, QT_TRANSLATE_NOOP("adm", "23.976 (Film)"), NULL},
    ...
    diaElemMenu menuStandardFps(&loadPicsFpsType, QT_TRANSLATE_NOOP("adm", "_Assign following standard frame rate:"),
                                NB_ITEMS(standardFpsEntries), standardFpsEntries);
    ...
    diaElemUInteger uintDefaultPortAvisynth(&defaultPortAvisynth, QT_TRANSLATE_NOOP("adm", "Default port to use"), 1024,
                                            65535);
...
#define IDX_WRITE_TO_DISK 2
    diaMenuEntry indexingEntries[] = {{IDX_FORCE_MEM, QT_TRANSLATE_NOOP("adm", "Disabled"), NULL},
                                      {IDX_USE_SAVED, QT_TRANSLATE_NOOP("adm", "Read-only"), NULL},
                                      {IDX_WRITE_TO_DISK, QT_TRANSLATE_NOOP("adm", "Enabled"), NULL}};

    diaElemMenu menuIndexingPs(&idxPsFiles, QT_TRANSLATE_NOOP("adm", "MPEG-PS files:"), NB_ITEMS(indexingEntries),
                               indexingEntries);
#if 0
    diaElemMenu menuIndexingTs(&idxTsFiles, QT_TRANSLATE_NOOP("adm","MPEG-TS files:"), NB_ITEMS(indexingEntries), indexingEntries);
#endif
    diaElemMenu menuIndexingMkv(&idxMkvFiles, QT_TRANSLATE_NOOP("adm", "Matroska files:"), NB_ITEMS(indexingEntries),
                                indexingEntries);
    diaElemMenu menuIndexingMp4(&idxMp4Files, QT_TRANSLATE_NOOP("adm", "MP4 files:"), NB_ITEMS(indexingEntries),
                                indexingEntries);
    ...
    diaMenuEntry videoMode[] = {{RENDER_GTK, getNativeRendererDesc(0),
                                 QT_TRANSLATE_NOOP("adm", "This renderer provides no hardware acceleration")}
#ifdef USE_XV
                                ,
    ...
    diaMenuEntry msgEntries[] = {{0, QT_TRANSLATE_NOOP("adm", "No alerts"), NULL},
...
#if defined(ALSA_SUPPORT) || defined(OSS_SUPPORT)
    diaMenuEntry volumeEntries[] = {{0, QT_TRANSLATE_NOOP("adm", "PCM"), NULL},
                                    {1, QT_TRANSLATE_NOOP("adm", "Master"), NULL}};
    diaElemMenu menuVolume(&useMaster, QT_TRANSLATE_NOOP("adm", "_Volume control:"), NB_ITEMS(volumeEntries),
                           volumeEntries);
#endif

    diaMenuEntry mixerEntries[] = {{0, QT_TRANSLATE_NOOP("adm", "No downmixing"), NULL},
    ...
    diaElemMenu menuMixer(&downmix, QT_TRANSLATE_NOOP("adm", "_Local playback downmixing:"), NB_ITEMS(mixerEntries),
                          mixerEntries);
    //*********** AV_
    ...
    int currentIndex = 0;
    if (!prefs->get(DEFAULT_LANGUAGE, currentLanguage))
        currentLanguage = std::string("auto");
...
#define NB_ELEM(x) sizeof(x) / sizeof(diaElem *)
    /* User Interface */
    diaElem *diaUser[] = {&menuMessage,    &menuLanguage,   &resetEncoder, &enableAltShortcuts,
                          &swapUpDownKeys, &swapMouseWheel, &swapMarkers,  &checkForUpdate};
...
//#elif defined(OSS_SUPPORT)
    ...
    diaElemMenu menuToneMapHDR(&toneMappingHDR, QT_TRANSLATE_NOOP("adm", "Default _tone mapping method:"),
                               NB_ITEMS(toneMapEntries), toneMapEntries);

    ELEM_TYPE_FLOAT dTargetLumHDR = targetLumHDR;
    diaElemFloatResettable floatTargetLumHDR(&dTargetLumHDR, QT_TRANSLATE_NOOP("adm", "Target peak luminance (nits):"),
                                             0., 1000., DEFAULT_TARGET_LUMINANCE_HDR);
    diaMenuEntry outOfGamutEntries[] = {{0, QT_TRANSLATE_NOOP("adm", "Clipping"), NULL},
                                        {1, QT_TRANSLATE_NOOP("adm", "Compression"), NULL}};
    diaElemMenu menuOutOfGamutHDR(&outOfGamutHDR, QT_TRANSLATE_NOOP("adm", "Default out of gamut handling:"),
                                  NB_ITEMS(outOfGamutEntries), outOfGamutEntries);
    ...
    diaElemToggle filterPreviewAccelToggle(&filterPreviewAccel,
                                           QT_TRANSLATE_NOOP("adm", "Enable OpenGL display in filter preview dialogs"));
    diaElemReadOnlyText openGlText(
        NULL, QT_TRANSLATE_NOOP("adm", "OpenGL video display and filters require "
    ...
    diaElemToggle dxva2OverrideVersion(&bdxva2_override_version,
                                       QT_TRANSLATE_NOOP("adm", "Ignore driver blacklist (Intel)"));
    diaElemToggle dxva2OverrideProfile(&bdxva2_override_profile,
                                       QT_TRANSLATE_NOOP("adm", "Ignore codec blacklist (Intel, HEVC 10bit)"));
    ...
    diaElemReadOnlyText hwAccelText(
        NULL, QT_TRANSLATE_NOOP("adm", "If you use Hw decoding, it is better to use the matching display driver"),
        NULL);
#endif
    diaElemReadOnlyText hwAccelMultiThreadText(
        NULL,
        QT_TRANSLATE_NOOP("adm", "Enabling Hw decoding disables multi-threading, restart application to apply changes"),
        NULL);
    ...
    diaElemTabs *tabs[] = {&tabUser,       &tabImport,   &tabOutput, &tabAudio, &tabVideo, &tabPostProc,
#ifdef HW_ACCELERATED_DECODING
                           &tabHwDecoding,
#endif
                           &tabCpu,        &tabThreading};
...
#if defined(HW_ACCELERATED_DECODING) && !defined(USE_VIDEOTOOLBOX)
#if !defined(USE_DXVA2)
#ifdef USE_VDPAU
...
#define DOME(x, y)                                                                                                     \
    if (y)                                                                                                             \
        ...
        if (bvdpau)
            lavcThreads = 1; // disable multi-threaded decoding
#endif
#ifdef USE_DXVA2
        // DXVA2
