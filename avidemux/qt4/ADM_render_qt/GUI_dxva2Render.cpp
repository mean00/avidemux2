...
bool dxvaRender::init(GUI_WindowInfo *window, uint32_t w, uint32_t h, float zoom)
{
    ...
    if (ADM_FAILED(D3DCall(IDirect3D9, GetAdapterDisplayMode, d3dHandle, D3DADAPTER_DEFAULT, &displayMode)))
    {
    ...
    if (ADM_FAILED(
            D3DCall(IDirect3D9, GetDeviceCaps, d3dHandle, D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, &deviceCapabilities)))
    {
...
bool dxvaRender::setup()
{
    ...
    presentationParameters.SwapEffect =
        D3DSWAPEFFECT_DISCARD; // We could use copy, but discard seems faster according to ms
    ...
    if (admD3D::isDirect9Ex())
    {
        ...
        if (ADM_FAILED(D3DCall(IDirect3D9Ex, CreateDeviceEx, ((IDirect3D9Ex *)d3dHandle), D3DADAPTER_DEFAULT,
                               D3DDEVTYPE_HAL, presentationParameters.hDeviceWindow,
                               D3DCREATE_SOFTWARE_VERTEXPROCESSING, &presentationParameters, NULL, &d3d9deviceex)))
        {
    ...
    {
        if (ADM_FAILED(D3DCall(IDirect3D9, CreateDevice, d3dHandle, D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL,
                               presentationParameters.hDeviceWindow, D3DCREATE_SOFTWARE_VERTEXPROCESSING,
    ...
    if (ADM_FAILED(D3DCall(IDirect3DDevice9, CreateOffscreenPlainSurface, d3dDevice, displayWidth, displayHeight,
    ...
    {
        D3DFORMAT yv12 = (D3DFORMAT)MAKEFOURCC('Y', 'V', '1', '2');
        if (ADM_FAILED(D3DCall(IDirect3DDevice9, CreateOffscreenPlainSurface, d3dDevice, imageWidth, imageHeight, yv12,
                               D3DPOOL_DEFAULT, &myYV12Surface, NULL)))
        {
    ...
    scaler = new ADMColorScalerFull(ADM_CS_BICUBIC, imageWidth, imageHeight, displayWidth, displayHeight,
                                    ADM_PIXFRMT_YV12, ADM_PIXFRMT_RGB32A);
    panScan.left = 0;
...
static bool d3dBlit(ADMImage *pic, ADM_PLANE plane, uint8_t *target, int targetPitch, int w, int h)
{
    ...
    BitBlit(target, targetPitch, y, pitch, w, h);
    return true;
}
/**
...
bool dxvaRender::displayImage_yv12(ADMImage *pic)
{
    ...
    // upload....
    if (ADM_FAILED(IDirect3DDevice9_GetBackBuffer(d3dDevice, 0, 0, D3DBACKBUFFER_TYPE_MONO, &bBuffer)))
    {
    ...
    if (ADM_FAILED(IDirect3DDevice9_StretchRect(d3dDevice, myYV12Surface, NULL, bBuffer, NULL, D3DTEXF_LINEAR)))
    {
