...
bool XvRender::displayImage(ADMImage *src)
{
    if (xvimage)
    {
        ...
        BitBlit((uint8_t *)xvimage->data, xvimage->pitches[0], src->GetReadPtr(PLANAR_Y), src->GetPitch(PLANAR_Y),
                imageWidth, imageHeight);
        BitBlit((uint8_t *)xvimage->data + plane, xvimage->pitches[1], src->GetReadPtr(PLANAR_V),
                src->GetPitch(PLANAR_V), imageWidth / 2, imageHeight / 2);
        BitBlit((uint8_t *)xvimage->data + plane + plane2, xvimage->pitches[2], src->GetReadPtr(PLANAR_U),
                src->GetPitch(PLANAR_U), imageWidth / 2, imageHeight / 2);
        XUnlockDisplay(xv_display);
...
bool XvRender::lowLevelXvInit(GUI_WindowInfo *window, uint32_t w, uint32_t h)
{
    ...
    if (Success != XvQueryAdaptors(WDN, DefaultRootWindow(WDN), &adaptors, &ai))
    {
    ...
    }
    {
        ...
        xvimage = XvShmCreateImage(WDN, xv_port, xv_format, 0, w, h, &Shminfo);
...
#define CHECK(x) (curai->type & x) ? (" | " #x) : ""
    ADM_info("[Xvideo] Type              : %d%s%s%s%s%s\n", curai->type, CHECK(XvInputMask), CHECK(XvOutputMask),
             CHECK(XvVideoMask), CHECK(XvStillMask), CHECK(XvImageMask));
