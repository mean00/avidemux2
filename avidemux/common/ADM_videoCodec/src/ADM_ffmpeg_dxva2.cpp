...
bool dxva2Probe(void)
{
    ...
    if (admCoreCodecSupports(ADM_CORE_CODEC_FEATURE_DXVA2) == false)
    {
        GUI_Error_HIG(QT_TRANSLATE_NOOP("adm", "Error"),
                      QT_TRANSLATE_NOOP("adm", "Core has been compiled without DXVA2 support, but the application has "
                                               "been compiled with it.\nInstallation mismatch"));
        dxva2Working = false;
        return false;
    }

    if (false == admDxva2::init(&xinfo))
        return false;
    dxva2Working = true;
    return true;
}
...
{
    char str[49];
    snprintf(str, 48, "%u.%u.%u.%u", (version >> 48) & 0xFFFF, (version >> 32) & 0xFFFF, (version >> 16) & 0xFFFF,
             version & 0xFFFF);
    str[48] = 0;
 ...
 * @param bpp
 */
decoderFFDXVA2::decoderFFDXVA2(AVCodecContext *avctx, decoderFF *parent) : ADM_acceleratedDecoderFF(avctx, parent)
{
    ...
    if (cacheSize > EDITOR_CACHE_MAX_SIZE)
        cacheSize = EDITOR_CACHE_MAX_SIZE;
    if (cacheSize < EDITOR_CACHE_MIN_SIZE)
        cacheSize = EDITOR_CACHE_MIN_SIZE;
    ...
    case AV_CODEC_ID_AV1: // very unsure
        num_surfaces += 8;
        break;
    default:
        num_surfaces += 2;
    ...
    bool er = false;
    if (!admDxva2::allocateDecoderSurface(this, avctx->coded_width, avctx->coded_height, num_surfaces, surfaces,
                                          dmSurfaces, bits))
    {
    ...
    dx_context->decoder =
        admDxva2::createDecoder(avctx->codec_id, avctx->coded_width, avctx->coded_height, num_surfaces, surfaces, bits);
    aprintf("Decoder=%p\n", dx_context->decoder);
...
int decoderFFDXVA2::getBuffer(AVCodecContext *avctx, AVFrame *pic)
{
    ...
    pic->buf[0] = av_buffer_create((uint8_t *)&(s->surface), // Maybe a memleak here...
                                   sizeof(s->surface), ADM_DXVA2releaseBuffer, (void *)s, AV_BUFFER_FLAG_READONLY);
 ...
 * @return
 */
bool ADM_hwAccelEntryDxva2::canSupportThis(struct AVCodecContext *avctx, const enum AVPixelFormat *fmt,
                                           enum AVPixelFormat &outputFormat)
{
    bool enabled = false;
...
ADM_acceleratedDecoderFF *ADM_hwAccelEntryDxva2::spawn(struct AVCodecContext *avctx, const enum AVPixelFormat *fmt)
{
    ...
    else
    {
        ADM_info("Spawned DXVA2 decoder with %d surfaces, avcontext=%p, ff=%p, instance=%p\n",
                 instance->getNumSurfaces(), avctx, ff, instance);
    }
    return instance;
}
