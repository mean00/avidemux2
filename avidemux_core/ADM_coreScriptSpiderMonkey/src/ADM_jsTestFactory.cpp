...
extern "C"
{
	...
	*/
	...
	{
		uint32_t tog=0;
		diaElemUInteger blend(&tog,"Uinteger",0,255);
		diaElem *elems[]={&blend   };
	...
	*/
	int jsTestFacFloat(void)
	{
		...
		diaElemFloat blend(&tog,"Float",0,255);
		diaElem *elems[]={&blend   };
	...
	*/
	int jsTestFacToggle(void)
	{
		bool tog=0;
		uint32_t test=0;
		diaElemToggle blend(&tog,"Toggle");
	...
	*/
	int jsTestFacMenu(void)
	{
		...
		diaMenuEntry menu[]={
			...
			{4,     "3:2 Pulldown",NULL},
			{6,     "Pal/Secam",NULL},
			{7,  "NTSC converted from PAL",NULL}
		};
		diaElemMenu blend(&tog,"menu",4,menu);
